...
namespace Utils {
	...
	cv::Point mapFromMainScreenToDebugCoordinates(cv::Point point) {
		...
		std::cout << "Geometry conversion, geometry: " << geometry->width << "x" << geometry->height << std::endl;
		std::cout << "Debug size: " << Application::Components::videoInput->debugFrame.size().width << "x" << Application::Components::videoInput->debugFrame.size().height << std::endl << std::endl;
		...
		std::cout << "Orig point: " << point.x << "x" << point.y << std::endl;
		std::cout << "Mapped point: " << xFactor*point.x << "x" << yFactor*point.y << std::endl<< std::endl;
